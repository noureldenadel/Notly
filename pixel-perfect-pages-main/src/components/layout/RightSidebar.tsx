...
const BacklinkItem = ({ title, type, date }: { title: string; type: string; date: string }) => (
  <button className="flex items-start gap-1.5 w-full p-1.5 hover:bg-accent/50 rounded-md transition-colors text-left group">
    ...
    <div className="flex-1 min-w-0">
      ...
      <p className="text-[10px] text-muted-foreground">{type} • {date}</p>
    </div>
    <ChevronRight className="w-2.5 h-2.5 text-muted-foreground opacity-0 group-hover:opacity-100 mt-1" />
  </button>
);
...
export const RightSidebar = ({ isOpen, onClose }: RightSidebarProps) => {
  ...
  return (
    <TooltipProvider delayDuration={300}>
      <div className="w-56 h-full bg-card border-l border-border flex flex-col animate-slide-in-right">
        {/* Header */}
            ...
            <TabsList className="h-7 p-0.5 bg-muted">
              <Tooltip>
                <TooltipTrigger asChild>
                  <TabsTrigger value="info" className="h-6 w-6 p-0 data-[state=active]:bg-card">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <TabsTrigger value="library" className="h-6 w-6 p-0 data-[state=active]:bg-card">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <TabsTrigger value="highlights" className="h-6 w-6 p-0 data-[state=active]:bg-card">
                    <Highlighter className="w-3.5 h-3.5" />
                  </TabsTrigger>
                </TooltipTrigger>
                <TooltipContent side="bottom">Highlights</TooltipContent>
              </Tooltip>
              <Tooltip>
                <TooltipTrigger asChild>
                  <TabsTrigger value="boards" className="h-6 w-6 p-0 data-[state=active]:bg-card">
                    <Layers className="w-3.5 h-3.5" />
                  </TabsTrigger>
                </TooltipTrigger>
                <TooltipContent side="bottom">Boards</TooltipContent>
              </Tooltip>
            </TabsList>
          </Tabs>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="icon" className="w-6 h-6 ml-1" onClick={onClose}>
                <X className="w-3 h-3" />
              </Button>
            </TooltipTrigger>
